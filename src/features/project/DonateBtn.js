...
function DonateBtn({ project }) {
  ...
  const handleDonate = () => {
    auth?.user ? navigate(`/projects/${project._id}/donation/${auth?.user?._id}`) : navigate("/auth/login");
  };

  return (
    <Stack width={{ xs: "100%", md: "25%" }} padding={1} textAlign="center" justifyContent="space-between">
      <Box>
      ...
      {/* <Donation amount={donationAmount} onAmountChange={handleDonationAmountChange} /> */}
